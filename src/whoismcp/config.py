...
class Config:
    ...
    rdap_timeout: int = field(default_factory=lambda: int(os.getenv("RDAP_TIMEOUT", "30")))
    ...
    cache_cleanup_interval: int = field(
        default_factory=lambda: int(os.getenv("CACHE_CLEANUP_INTERVAL", "300"))  # 5 minutes
    )
    ...
    def validate(self) -> None:
        ...
        if self.global_rate_limit_per_second <= 0:
            raise ValueError(f"Invalid global rate limit: {self.global_rate_limit_per_second}")
        
        if self.client_rate_limit_per_second <= 0:
            raise ValueError(f"Invalid client rate limit: {self.client_rate_limit_per_second}")
