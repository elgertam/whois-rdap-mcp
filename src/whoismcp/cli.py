    ...
    async def run_whois() -> None:
        ...
        try:
            if is_valid_domain(target):
                ...
                click.echo(f"Error: Invalid target '{target}'. Must be a domain or IP address.", err=True)
                sys.exit(1)
                ...
                if result.get('success'):
                    ...
                    if response_data:
                        ...
                        for event in events:
                            ...
                            if action and date:
                                click.echo(f"{action.replace('_', ' ').title()}: {date}")
                else:
                    click.echo(f"Error: {result.get('error', 'Unknown error')}", err=True)
                    sys.exit(1)
    ...
    async def run_test() -> None:
        ...
        try:
            async with anyio.connect_tcp(host, port) as stream:
                ...
                test_msg = {"jsonrpc": "2.0", "method": "initialize", "id": 1, "params": {}}
                ...
                if 'result' in response:
                    ...
                    click.echo(f"Server: {response['result'].get('serverInfo', {}).get('name', 'Unknown')}")
                else:
...
@cli.command()
@click.argument('target')
@click.option('--method', type=click.Choice(['whois', 'rdap']), default='whois',
              help='Lookup method to test')
@click.option('--host', default='127.0.0.1', help='MCP server host')
@click.option('--port', default=5001, help='MCP server port')
@click.pass_context
def test_lookup(ctx: click.Context, target: str, method: str, host: str, port: int) -> None:
        ...
        try:
            async with anyio.connect_tcp(host, port) as stream:
                ...
                if 'result' in response and 'content' in response['result']:
                    ...
                    if result.get('success'):
                        server_key = f"{method}_server" if method == "rdap" else "whois_server"
