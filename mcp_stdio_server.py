...
class MCPStdioServer:
    ...
    async def _handle_rdap_lookup(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Check rate limiting
        if not await self.rate_limiter.check_rate_limit("mcp_client"):
            return {
                "isError": True,
                "content": [{
                    "type": "text",
        ...
        # Perform RDAP lookup
        result = await self.rdap_service.lookup(target)
    ...
    async def handle_read_resource(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if uri.startswith("whois://domain/"):
                ...
                result = await self.whois_service.lookup(domain)
                return {
                    "contents": [{
                        "uri": uri,
                        "mimeType": "application/json",
                ...
                result = await self.whois_service.lookup(ip)
                return {
                    "contents": [{
                        "uri": uri,
                        "mimeType": "application/json",
                ...
                result = await self.rdap_service.lookup(domain)
                return {
                    "contents": [{
                        "uri": uri,
                        "mimeType": "application/json",
                ...
                result = await self.rdap_service.lookup(ip)
                return {
                    "contents": [{
                        "uri": uri,
                        "mimeType": "application/json",
